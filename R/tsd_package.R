...
tsd_package_install <- function(zip_file,
  ...
  # if(!is.null(lib)) opts <- c(opts, "lib = '", lib, "'")
  ...
  for(k in 1:length(pkgs)){
    if(verbose) cat("installing", pkgs[k], sep = " ")
    log_file <- paste0(pkgs[k], format(Sys.time(), "%Y%m%d-%H%M%S.txt"))
    jj <- file.create(log_file)

    opts2 <- paste(opts,
                   package_workarounds_install(pkgs[k], folder))

    cmd <- paste("CMD INSTALL --library=", lib, opts2, pkgs[k])
    system2("R", cmd ,
            stdout = log_file, stderr = log_file)

    ff[[k]] <- readLines(log_file)
  ...
  errs <- lapply(which(!ff2), function(x) ff[[x]])
